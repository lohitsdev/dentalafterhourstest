...
// Existing SendGrid configuration test endpoint
app.get('/test/sendgrid-config', (req, res) => {
  const sgMail = require('@sendgrid/mail');
  ...
  console.log('🧪 Testing SendGrid configuration:', config);
  ...
  if (process.env.SENDGRID_API_KEY) {
    ...
    sgMail.client.request({
      ...
      res.json({ ...config, status: 'API Key Valid' });
    }).catch(error => {
      config.apiKeyValid = false;
      config.error = error.message;
      config.errorDetails = error.response?.body;
      console.error('❌ SendGrid API key validation failed:', error.message);
      res.json({ ...config, status: 'API Key Invalid' });
    });
  } else {
    console.log('❌ SendGrid API key is missing');
    res.json({ ...config, status: 'API Key Missing' });
  }
});
